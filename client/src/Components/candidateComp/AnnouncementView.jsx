...
export default function AnnouncementView({visible, onClose, announcement}) {
  const { users } = useContext(UserContext);
    if (!visible || !announcement) return null;
    //find the user with the same email as status.email
    const user = users.find(user => user.email === announcement.user_email);
  return (
      ...
      <div className="bg-[#19191A] p-6 rounded-lg shadow-lg h-fit w-auto border-orange-700 border-[1px] ">
        ...
        <div className='flex justify-left'>
          ...
          <div className='my-auto ml-5 text-start'>
            <h1>{announcement.user_fname} {announcement.user_lname}</h1>
