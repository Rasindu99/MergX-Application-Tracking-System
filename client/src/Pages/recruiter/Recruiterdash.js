...
export default function Recruiterdash() {
  ...
  const getacceptedapplications = async () => {
    try {
      const response = await axios.get(
        "/reporting/getallacceptedapplicationcount"
      );
      setCardDetails((prevState) => ({
        ...prevState,
  ...
  return (
    <div className="flex items-center ml-[320px] justify-center ">
      <div
        className="rounded-3xl  w-full mx-5 mt-2"
        style={{
          background:
            "linear-gradient(to bottom, #2B2B2B 0%, rgba(43, 43, 43, 0) 35%)",
        }}
      >
        ...
        <div className="flex items-center justify-around mt-10">
          ...
          <CardL
            ...
            val={carddetails.acceptedApplications}
          />
          <CardL
            name="Today"
            subName="Interviews"
            val={carddetails.todayinterveiw}
          />
        ...
        <div className="flex items-center justify-around mt-10">
          ...
          <CardS
            name="Interviewed"
            subName="Candidates"
            val={carddetails.interviewedcandidates}
          />
          <CardS
            name="Total"
            subName="Applications"
            val={carddetails.applications}
          />
        </div>
        </div>
        ...
        {/* <div className="flex flex-col items-center mt-20">
          <div
            className="w-[60%] flex items-center justify-center mb-4 p-4 rounded-2xl border-2 border-[#2B2B2B]"
            style={{
              background:
                "linear-gradient(to bottom, #2B2B2B 0%, rgba(43, 43, 43, 0) 80%)",
            }}
          >
            <input
              type="text"
          ...
          <div className="w-[75%] max-h-[700px] overflow-y-auto pr-2">
            {filteredCandidates.map((candidate, index) => (
              <div
                key={index}
                className="flex items-center justify-between p-4 mb-2 rounded border-x-2 border-[#2B2B2B]"
                style={{
                  background:
                    "linear-gradient(to bottom, #2B2B2B 0%, rgba(43, 43, 43, 0) 60%)",
                }}
              >
                <div className="items-center block ">
                  <img
                    src={candidate.image}
                    alt=""
                    className="rounded-full border-[2px] w-[50px] h-[50px]"
                  />
                </div>
                <div>
                ...
                <div>
                  <p className="text-sm text-white opacity-25">
